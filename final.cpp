...
public:
    ...
    void search( int id ) {
        ...
        int dig = firstDigit(id);
        
        do {
            
            probe = ( hash(dig) + i * i ) % m;
            if( ( *table )[probe].isEmpty ) {
                cout << "Doesn't Exist" << endl;
                return;
            }
            else if ((*table )[probe].getID() == id) {
                cout << endl << " [ index : " << probe << " | " << ( *table )[probe].getName() << " , " << ( *table )[probe].getID() <<
                " ] " << endl;
                return;
            }
            else
    ...
    int firstDigit(int x) {
        if (x == 0) return 0;
        x = abs(x);
        return (int) floor(x / pow(10, floor(log10(x))));
    }
    ...
    void insert(string name, int id ) {
        ...
        } while(!( *table )[probe].isEmpty && enrollmentCapacity < (*table).size()  );
    ...
    void insert( Student obj ) {
        ...
        } while( !( *table )[probe].isEmpty && enrollmentCapacity < (*table).size() && i != m );
        if( pre_inserts == numInserts ) {
            cout << "Fail on Insertion(Maximum number of probes[" << LIMIT << "] exceeded)." << endl;
        }
